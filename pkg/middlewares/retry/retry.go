...
func (b *Retry) Schema() (string, error) {

	return "", nil
}

// New returns a new retry middleware.
func New(ctx context.Context, next http.Handler, config *Retry, listener Listener, name string) (http.Handler, error) {
...
func (r *retry) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	logger := logs.GetLogger(req.Context(), r.name, TypeName)
